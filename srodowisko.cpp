...
void Srodowisko::lokuj(Miszkaniec *mieszkaniec, unsigned int wiersz, unsigned int kolumna)
{
    if (wiesz<wiersze && kolumna < kolumny)
...
Sasiedztwo Srodowisko::ustalSasiedztwo(unsigned int wiersz, unsigned int kolumna) const
{
    ...
    for(Polozenie p : {P, PG, G, LG, L, LD, D, PD})
    {
        ...
        {
            sasiedztow.okreslSasiada(p, nisza[wiersz1][kolumna1].ktoTuMieszka());
        }
    }
    return sasiedztwo;
}
...
void Srodowisko::wykonajAkcje(unsigned int wiersz, unsigned int kolumna)
{
    ...
    Sasiedztwo sasiedztwo = ustalSasiedzto(wiersz, kolumna);
...
std::ostream &operator<<(std::ostream &strumien, Srodowisko & srodowisko)
{
